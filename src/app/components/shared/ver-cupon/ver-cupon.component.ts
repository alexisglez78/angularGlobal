...
export class VerCuponComponent implements OnInit, OnChanges {
  ...
  getCupon(){
    this.wsCupones.getCupones(1,5,'').subscribe((data:any)=> {
     ...
     getCupones(pagina: number = 1, cantidad,search) {
      this.cupones = [];
      this.wsCupones.getCupones(pagina, cantidad,search).subscribe((data: any) => {
          if (data.error) {
              return;
          }
   ...
   verPagina(pagina) {
    ...
    buscar(value){
        ...
        this.paginaActiva=1;
        this.init=false;
        this.getCupones(this.paginaActiva,this.cantidad,value);
